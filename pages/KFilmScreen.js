...
const KFilmScreen = ({ navigation }) => {
    ...
    const fetchData = async () => {
        if (!loading) {
            setLoading(true);
            try {
                const res = await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=b2bd88d5ce5e168ce72a6d589ceab4a5&language=en-US&region=KR&sort_by=popularity.desc&include_adult=false&page=${page}&without_genres=10749%2C18&with_runtime.gte=1&with_original_language=ko`);
        ...
        if (query !== "") {
            const filteredTitles = data.filter(
                (title) =>
                    ...
                    title.original_title.toLowerCase().includes(query.toLowerCase())
            );
    ...
    const nextPage = async () => {
        if (!loading) {
            setLoading(true);
            try {
                const res = await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=b2bd88d5ce5e168ce72a6d589ceab4a5&language=en-US&region=KR&sort_by=popularity.desc&include_adult=false&page=${page + 1}&without_genres=10749%2C18&with_runtime.gte=1&with_original_language=ko`);
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={styles.header}>
                <Text style={styles.headerText}>Explore KFilms</Text>
                <TextInput
                    style={styles.searchInput}
                    placeholder="Search KFilms"
